...
import type { DiscoveredConfig, Logger } from './types'
 ...
 */
export const discoverViaProviderList = async (
  domain: string,
  logger: Logger
  ...
  for (const provider of imapProviders) {
    ...
    if (matchingDomain) {
      logger.info(
        `[Discovery/S1-Providers] Found pattern match provider: ${provider.name} (pattern: ${matchingDomain})`
      )
  ...
  logger.info(
    `[Discovery/S1-Providers] No known provider found, trying fallback patterns for ${domain}`
  )
  return await tryFallbackPatterns(domain, logger)
}

/**
 * Validates a discovered config and returns it if valid.
 */
const validateAndReturnConfig = async (
  provider: { name: string; config: DiscoveredConfig },
  logger: Logger
): Promise<DiscoveredConfig | null> => {
  if (provider?.config?.imap?.host && provider?.config?.imap?.port) {
    const imapWorks = await isValidEmailServer(
 ...
 */
const tryFallbackPatterns = async (
  domain: string,
  logger: Logger
): Promise<DiscoveredConfig | null> => {
  const fallbackPatterns = [`imap.${domain}`, `mail.${domain}`, `mx.${domain}`]
  ...
  for (const host of fallbackPatterns) {
    ...
    if (isValid) {
      ...
      return {
        ...
        smtp: {
          host: host.replace('imap.', 'smtp.').replace('mail.', 'smtp.').replace('mx.', 'smtp.'),
          port: 465,
          secure: true,
        },
      }
