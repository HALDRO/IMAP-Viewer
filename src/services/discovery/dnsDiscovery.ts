...
import {
  isValidEmailHost,
  isValidEmailHostWithRealHostname,
  isValidEmailServer,
} from './connectionTesting'
import type { DiscoveredConfig, Logger, ServerConfig } from './types'
...
const generateHostnamesFromMX = (mxHostname: string, _domain: string): string[] => {
  ...
  const prefixReplacements = [
    { from: /^mail\./, to: ['imap.', 'smtp.'] },
    { from: /^smtp\./, to: ['imap.', 'mail.'] },
...
const getMXBasedHostnames = async (domain: string, logger: Logger): Promise<string[]> => {
  try {
    ...
    for (const mx of mxRecords) {
      logger.info(
        `[Discovery/S2-DNS-MX] Found MX record: ${mx.exchange} (priority: ${mx.priority})`
      )
      const hostsFromMX = generateHostnamesFromMX(mx.exchange, domain)
      for (const host of hostsFromMX) {
        allHosts.add(host)
      }
    }
    ...
    logger.info(
      `[Discovery/S2-DNS-MX] Generated ${result.length} hostnames from MX records: ${result.join(', ')}`
    )
    return result
  } catch (error) {
...
export const discoverViaDns = async (
  ...
  logger.info(
    `[Discovery/S2-DNS] Will test hosts: ${[
      `imap.${domain}`,
      `mail.${domain}`,
      `smtp.${domain}`,
      `mx.${domain}`,
 ...
 */
const discoverViaSrvRecords = async (
  domain: string,
  logger: Logger
): Promise<DiscoveredConfig | null> => {
      ...
      try {
        ...
        if (records && records.length > 0) {
          ...
          for (const record of records) {
            ...
            const secure = srv.includes('s') // _imaps imply SSL/TLS

            const isValid = await isValidEmailServer(name, port, secure, logger)
            if (isValid) {
              logger.info(
                `[Discovery/S2-DNS-SRV] Found working ${service.toUpperCase()} server: ${name}:${port}`
              )
        ...
        if (errorCode !== 'ENODATA' && errorCode !== 'ENOTFOUND') {
          logger.warn(
            `[Discovery/S2-DNS-SRV] SRV lookup for ${srv}.${domain} failed: ${errorMessage}`
          )
        }
      }
      return null
  ...
  for (const res of resolutions) {
    if (res) {
      if (res.service === 'imap' && res.config) {
        discovered.imap = res.config
      }
    }
  }

  return discovered.imap ? discovered : null
}

/**
 * Fallback Strategy: DNS-based guessing of common server names.
 */
const discoverViaDnsGuessing = async (
  domain: string,
  logger: Logger
): Promise<DiscoveredConfig | null> => {
  ...
  logger.info(
    `[Discovery/S2-DNS-Guess] Testing ${allHosts.length} hostnames: ${allHosts.join(', ')}`
  )
  const imapConfigs = [
  ...
  const testConfig = async (
    host: string,
    configs: { port: number; secure: boolean }[],
    type: 'imap' | 'smtp'
    ...
    try {
      ...
      for (const config of configs) {
        logger.info(
          `[Discovery/S2-DNS-Guess] Testing ${host}:${config.port} (secure: ${config.secure})`
        )

        // Add timeout to each host test
        const testPromise = isValidEmailHostWithRealHostname(
          host,
          config.port,
          config.secure,
          logger
        )
        const timeoutPromise = new Promise<{ isValid: boolean; realHostname?: string }>(
          (_, reject) => setTimeout(() => reject(new Error('Host test timeout')), 8000)
        )
        ...
        if (result.isValid) {
          ...
          if (result.realHostname && (type === 'imap' || type === 'smtp')) {
            ...
            logger.info(
              `[Discovery/S2-DNS-Guess] Checking real hostname: "${realHost}" for type: "${type}"`
            )
            // If the real hostname doesn't look like a mail server, add appropriate prefix
            if (
              !realHost.includes('imap') &&
              !realHost.includes('mail') &&
              !realHost.includes('mx') &&
              !realHost.includes('smtp')
            ) {
              if (type === 'imap') {
                finalHost = `imap.${result.realHostname}`
                logger.info(
                  `[Discovery/S2-DNS-Guess] Real hostname doesn't look like mail server, using: ${finalHost}`
                )
              } else if (type === 'smtp') {
              ...
              logger.info(
                `[Discovery/S2-DNS-Guess] Using real hostname from TLS certificate: ${result.realHostname}`
              )
            }
          }

          logger.info(
            `[Discovery/S2-DNS-Guess] Found working ${type.toUpperCase()} server: ${finalHost}:${config.port}`
          )
      ...
      logger.info(
        `[Discovery/S2-DNS-Guess] Error testing ${type.toUpperCase()} host ${host}: ${error}`
      )
    }

    logger.info(
      `[Discovery/S2-DNS-Guess] No working ${type.toUpperCase()} server found for host: ${host}`
    )
    return null
  }

  logger.info(`[Discovery/S2-DNS-Guess] Starting testing of ${allHosts.length} hosts for IMAP/SMTP`)

  // Test IMAP first (priority), then SMTP
  const discovered: DiscoveredConfig = {}
  ...
  if (discovered.imap && discovered.smtp) {
    ...
    if (imapDomain !== smtpDomain) {
      logger.info(
        `[Discovery/S2-DNS-Guess] Hostname inconsistency detected: IMAP=${imapHost}, SMTP=${smtpHost}`
      )
      ...
      if (potentialSmtpHost !== smtpHost) {
        logger.info(
          `[Discovery/S2-DNS-Guess] Testing SMTP consistency: trying ${potentialSmtpHost} instead of ${smtpHost}`
        )

        // Test if the consistent hostname works for SMTP
        const testPromise = isValidEmailHostWithRealHostname(
          potentialSmtpHost,
          discovered.smtp.port,
          discovered.smtp.secure,
          logger
        )
        const timeoutPromise = new Promise<{ isValid: boolean; realHostname?: string }>(
          (_, reject) => setTimeout(() => reject(new Error('Consistency test timeout')), 5000)
        )
        ...
        try {
          ...
          if (result.isValid) {
            logger.info(
              `[Discovery/S2-DNS-Guess] SMTP hostname consistency fix: using ${potentialSmtpHost} instead of ${smtpHost}`
            )
            discovered.smtp.host = potentialSmtpHost
          } else {
            logger.info(
              `[Discovery/S2-DNS-Guess] SMTP hostname consistency test failed, keeping original: ${smtpHost}`
            )
          }
        } catch (_error) {
          logger.info(
            `[Discovery/S2-DNS-Guess] SMTP hostname consistency test error, keeping original: ${smtpHost}`
          )
        }
      }
    }
  ...
  const hasResults = discovered.imap
  logger.info(
    `[Discovery/S2-DNS-Guess] DNS guessing completed for ${domain}. Found: ${hasResults ? 'YES' : 'NO'}`
  )
  if (hasResults) {
    if (discovered.imap)
      logger.info(`[Discovery/S2-DNS-Guess] IMAP: ${discovered.imap.host}:${discovered.imap.port}`)
    if (discovered.smtp)
      logger.info(`[Discovery/S2-DNS-Guess] SMTP: ${discovered.smtp.host}:${discovered.smtp.port}`)
  }
