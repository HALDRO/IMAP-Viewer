 ...
 */
export const discoverViaExchangeAutodiscover = async (
  domain: string,
  logger: Logger
...
const getAutodiscoverEndpoints = async (domain: string, logger: Logger): Promise<string[]> => {
  ...
  try {
    ...
    if (errorCode !== 'ENODATA' && errorCode !== 'ENOTFOUND') {
      logger.warn(`[Discovery/S3-Exchange] SRV lookup failed: ${errorMessage}`)
    }
  }
 ...
 */
async function tryExchangeAutodiscoverUrl(
  url: string,
  domain: string,
  logger: Logger
): Promise<DiscoveredConfig | null> {
  try {
    ...
    logger.warn(
      `[Discovery/S3-Exchange] Request to ${url} failed: ${error instanceof Error ? error.message : 'Unknown error'}`
    )
    return null
  }
}
...
function parseExchangeAutodiscoverXml(xmlText: string, logger: Logger): DiscoveredConfig | null {
  try {
    ...
    if (server && type) {
      ...
      // Can be extended for SMTP if needed
    }

    return config.imap ? config : null
  } catch (error) {
    logger.error(
      `[Discovery/S3-Exchange] Failed to parse XML: ${error instanceof Error ? error.message : 'Unknown error'}`
    )
    return null
  }
}
